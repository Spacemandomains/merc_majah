...
function formatFullProfile(artist: Artist): string {
  ...
  if (artist.bookingEmail) {
    lines.push(`\n## Booking`);
    lines.push(`Email: ${artist.bookingEmail}`);
  }
  if (artist.pressEmail) {
    lines.push(`\n## Press Contact`);
    lines.push(`Email: ${artist.pressEmail}`);
  }
...
function createServer() {
  ...
  server.tool(
    ...
    async () => {
      try {
        ...
        if (!artist) {
          return {
            content: [{ type: "text", text: "Merc Majah's profile is not yet available in the database." }],
            isError: true,
          };
        }
        ...
        return {
          content: [{ type: "text", text: `Error fetching profile: ${String(err)}` }],
          isError: true,
        };
      }
    }
  ...
  server.tool(
    ...
    {
      short: z.boolean().optional().describe("If true, returns the short summary bio instead of the full biography"),
    },
    async ({ short }) => {
      try {
        const artist = await getMercMajah();
        if (!artist) {
          return { content: [{ type: "text", text: "Merc Majah's profile is not available." }], isError: true };
        }
        const lines: string[] = [`# ${artist.name} — Biography`];
  ...
  server.tool(
    ...
    async ({ type }) => {
      try {
        ...
        const discography = (artist.discography ?? []) as Discography;
        if (discography.length === 0) {
          return { content: [{ type: "text", text: `# ${artist.name} — Discography\n\nNo releases recorded yet.` }] };
        }
        let releases = discography;
        if (type) {
          releases = releases.filter((r) => r.type.toLowerCase() === type.toLowerCase());
        }
  ...
  server.tool(
    ...
    async () => {
      try {
        ...
        if (videos.length === 0) {
          return { content: [{ type: "text", text: `# ${artist.name} — Music Videos\n\nNo music videos recorded yet.` }] };
        }
  ...
  server.tool(
    ...
    async () => {
      try {
        ...
        const pressQuotes = (artist.pressQuotes ?? []) as PressQuote;
        if (pressQuotes.length === 0) {
          return { content: [{ type: "text", text: `# ${artist.name} — Press\n\nNo press quotes available yet.` }] };
        }
  ...
  server.tool(
    ...
    async () => {
      try {
        ...
        if (artist.bookingEmail) {
          lines.push(`**Booking:** ${artist.bookingEmail}`);
          hasContact = true;
        }
        if (artist.pressEmail) {
          lines.push(`**Press / Media:** ${artist.pressEmail}`);
          hasContact = true;
        }
        if (!hasContact) {
          lines.push("No contact information available yet.");
        }
        return { content: [{ type: "text", text: lines.join("\n") }] };
      } catch (err) {
        return { content: [{ type: "text", text: `Error: ${String(err)}` }], isError: true };
...
async function main() {
  ...
  app.get("/health", (_req: Request, res: Response) => {
    res.json({ status: "ok", server: "merc-majah-mcp", version: "1.0.0" });
  });

  app.get("/", (_req: Request, res: Response) => {
    res.json({
      name: "Merc Majah MCP Server",
      version: "1.0.0",
      description: "Model Context Protocol server for rap artist Merc Majah. Provides AI agents with authoritative information about the artist.",
      mcp_endpoint: "/mcp",
      tools: [
        { name: "get_profile", description: "Get the complete artist profile" },
        { name: "get_bio", description: "Get biography and background" },
        { name: "get_discography", description: "Get all releases with streaming links" },
        { name: "get_music_videos", description: "Get music videos with watch links" },
        { name: "get_social_links", description: "Get social media and streaming links" },
        { name: "get_press_quotes", description: "Get press and media coverage" },
        { name: "get_contact_info", description: "Get booking and press contact info" },
        { name: "get_merch", description: "Get official merchandise item with buy link" },
        { name: "buy_merchandise", description: "Get a direct Stripe checkout link to purchase merch" },
      ],
    });
  });
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Merc Majah MCP Server running on port ${PORT}`);
